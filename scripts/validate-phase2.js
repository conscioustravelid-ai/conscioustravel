...
for (const source of ["/company-outing", "/company-outing/", "/company-trip", "/company-trip/"]) {
  const redirect = vercel.routes.find((item) => new RegExp(item.src).test(source));
  check(redirect?.headers?.Location === "/corporate-packages/indonesia-region/", `${source}: redirect is invalid`);
  check(redirect?.status === 308, `${source}: redirect must be permanent`);
}
check(!vercel.redirects.some((item) => item.source.includes("company-outing") || item.source.includes("company-trip")), "Legacy redirects must use the early route phase");
