...
function articleHtml(post, posts) {
  ...
  const toc = createBlogToc(post.body, post.slug)
  const body = renderPortableText(post.body, {articleSlug:post.slug,headingIds:toc.headingIds})
  const tocTitleId = `blog-toc-title-${post.slug}`
  const tocHtml = renderBlogToc(toc, tocTitleId)
