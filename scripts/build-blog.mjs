...
function listingHtml(posts, categories) {
  ...
  const featuredBlock = featured ? `<section class="blog-featured section"><div class="container"><div class="blog-section-heading"><p class="blog-kicker">Pilihan terbaru</p><h2>Featured Article</h2></div><article class="blog-featured-card"><a class="blog-featured-media" href="/blog/${escapeAttribute(featured.slug)}/">${imageMarkup(featured, {loading:null, sizes:'(max-width: 900px) calc(100vw - 40px), 720px'}) || '<span class="blog-image-fallback" aria-hidden="true">Conscious Travel Journal</span>'}</a><div class="blog-featured-copy"><p class="blog-category">${escapeHtml(featured.category.name)}</p><h3><a href="/blog/${escapeAttribute(featured.slug)}/">${escapeHtml(featured.title)}</a></h3><p>${escapeHtml(featured.excerpt)}</p><p class="blog-meta">${escapeHtml(featured.author.name)} <span aria-hidden="true">·</span> ${escapeHtml(dateLabel(featured.publishedAt))}</p><a class="blog-read-link" href="/blog/${escapeAttribute(featured.slug)}/">Baca artikel <span aria-hidden="true">→</span></a></div></article></div></section>` : ''
  const latestBlock = latest.length ? `<section class="section blog-latest"><div class="container"><div class="blog-section-heading"><p class="blog-kicker">Jelajahi cerita</p><h2>Artikel Terbaru</h2></div><div class="blog-grid">${latest.map((post) => articleCard(post)).join('')}</div></div></section>` : ''
  ...
  const categoryBlock = categories.length ? `<section class="section blog-categories"><div class="container"><div class="blog-section-heading"><p class="blog-kicker">Topik perjalanan</p><h2>Kategori Blog</h2></div><ul>${categories.map((category) => `<li><span>${escapeHtml(category.name)}</span>${category.description ? `<small>${escapeHtml(category.description)}</small>` : ''}</li>`).join('')}</ul></div></section>` : ''
