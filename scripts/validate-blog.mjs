...
async function validatePage(relative, { article = false } = {}) {
  ...
  check(/blog\.css\?v=blog-performance-2/.test(html), `${relative}: stylesheet khusus Blog hilang`)
  check(!/\/css\/(?:variables|style|editorial|blog-foundation)\.css/.test(html), `${relative}: stylesheet website penuh masih dimuat`)
  check(/DM\+Sans:wght@400;600;700&family=DM\+Serif\+Display/.test(html), `${relative}: subset font Blog hilang`)
  check(!/Plus\+Jakarta\+Sans/.test(html), `${relative}: font website yang tidak digunakan masih dimuat`)
