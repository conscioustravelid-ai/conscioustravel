...
function itineraryHtml(block,{precedingHeadingId}={}){const labelId=precedingHeadingId||block.id;const title=precedingHeadingId?'':`<h3 id="${escapeAttribute(block.id)}" class="blog-itinerary-title">${escapeHtml(block.dayTitle)}</h3>`;return `<section class="blog-itinerary" aria-labelledby="${escapeAttribute(labelId)}">${title}<ol>${block.items.map((item)=>`<li><div class="blog-itinerary-time">${escapeHtml(item.time)}</div><div class="blog-itinerary-details"><h4>${escapeHtml(item.activity)}</h4>${item.optional?'<span class="blog-itinerary-optional">Opsional</span>':''}${item.area?`<p class="blog-itinerary-area">${escapeHtml(item.area)}</p>`:''}${item.notes?`<p class="blog-itinerary-notes">${escapeHtml(item.notes)}</p>`:''}</div></li>`).join('')}</ol></section>`}
...
export function renderReaderBlock(block,{articleSlug,precedingHeadingId}={}){
  if(block?._type==='table')return tableHtml(block)
  if(block?._type==='itineraryBlock')return itineraryHtml(block,{precedingHeadingId})
